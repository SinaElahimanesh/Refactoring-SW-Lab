            ...
            for (int j = 1; j < cols.length; j++) {
                if (!cols[j].equals("")) {
                    if (cols[j].equals("acc")) {
                        actionTable.get(actionTable.size() - 1).put(terminals.get(j), new AcceptAct(0));
                    } else if (terminals.containsKey(j)) {
//                        try {
                        Token t = terminals.get(j);
                        Action a = cols[j].charAt(0) == 'r' ? new ReduceAct(Integer.parseInt(cols[j].substring(1))) : new ShiftAct(Integer.parseInt(cols[j].substring(1)));
